...
class journal extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      orderids: localStorage.getItem('orderids'),
  ...
  testnumber = (e) => {
    if (e.keyCode === 13) {
      ...
      var range = localStorage.getItem('testrange').split('~')
      if (parseInt(range[0]) <= testnumber && testnumber <= parseInt(range[1])) {
        this.setState({
          result: '',
          color: "green"
        })
          ...
          <Layout>
            ...
            <Layout>
              ...
              <Content style={{ margin: '24px 16px', padding: 24, background: '#fff', minHeight: 280, paddingTop: '10px' }}>
                ...
                <Input
                    placeholder="请输入水表编号"
                    style={{ width: '20%', marginLeft: '10px', marginRight: '20px' }}
                    id="testnumber"
                    onKeyUp={this.testnumber}
                    onFocus={this.numberfocus}
                  />
                  <span style={{ marginRight: '20px' }}>工单号 : <span style={{ marginLeft: '10px', color: 'green', fontWeight: 'bold' }}>{this.state.orderids}</span> </span>
                  <span style={{ marginRight: '20px', }}>表号范围 : <span style={{ marginLeft: '10px', color: 'green', fontWeight: 'bold' }}>{this.state.testrange}</span> </span>
                </div>
                <div style={{ marginTop: '10px' }}>
                  <div style={{ textAlign: 'left', lineHeight: '40px' }}>

                    <span style={{ fontSize: '16px', fontWeight: 'bold' }}>测试结果 : <span style={{ color: this.state.color }}>{this.state.result}</span></span>
                  </div>
                  <Table
                    bordered
